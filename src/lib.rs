...
pub fn cluster(positions: Vec<f32>, threshold: f32, zoom_level: f32) -> Vec<i32> {
    ...
    let mut triangulation =
        DelaunayTriangulation::<Cluster, FloatKernel, DelaunayWalkLocate>::new();

    {
        let mut i = 0;
        for chunk in positions.chunks_exact(2) {
            let cluster = Cluster::new(chunk[0], chunk[1], zoom_level, i);
...
/// A cluster has a centroid and a list of members (IDs).
struct Cluster {
    point: Point2<f32>,
    members: Vec<i32>,
}
...
impl Cluster {
    ...
    /// - `lat`: Latitude
    /// - `lng`: Longitude
    /// - `zoom_level`: Zoom level (WebMercator)
    /// - `index`: unique ID of cluster
    fn new(lat: f32, lng: f32, zoom_level: f32, index: i32) -> Cluster {
        // to radians
        let lat = std::f32::consts::PI / 180_f32 * lat;
        let lng = std::f32::consts::PI / 180_f32 * lng;

        // to WebMercator projection
        let x = 256_f32 / (2_f32 * std::f32::consts::PI)
            * 2_f32.powf(zoom_level)
            * (lng + std::f32::consts::PI);
        let y = 256_f32 / (2_f32 * std::f32::consts::PI)
            * 2_f32.powf(zoom_level)
            * (std::f32::consts::PI - (std::f32::consts::PI * 0.25 + lat * 0.5).tan().ln());

        // cluster contains one member, at WebMercator position (x, y)
    ...
    /// Merge another cluster into `self`
    ///
    /// - `self`: Self
    /// - `other`: Other cluster to merge. Gets moved into `self`.
    fn merge(self: &mut Cluster, other: Cluster) {
        let la = self.members.len();
        let lb = other.members.len();

        let x: f32 =
            ((la as f32) * self.point.x + (lb as f32) * other.point.x) / ((la + lb) as f32);
        let y: f32 =
            ((la as f32) * self.point.y + (lb as f32) * other.point.y) / ((la + lb) as f32);
    ...
    /// Merge the two closest clusters, using an existing Delaunay triangulation.
    ///
    /// - `triangulation`: Clusters in an existing Delaunay triangulation.
    ///
    /// Returns either the two cluster centroids and their distance squared, or `None` if no two
    /// clusters remain.
    fn merge_closest(
        triangulation: &mut DelaunayTriangulation<Cluster, FloatKernel, DelaunayWalkLocate>,
    ) -> Option<(Point2<f32>, Point2<f32>, f32)> {
        ...
        match (a, b) {
            (Some(va), Some(vb)) => Some((va.point, vb.point, dist)),
...
impl HasPosition for Cluster {
    ...
    /// Get `Point2` position from a `Cluster` type.
